...
- Uses `tsconfig.precommit.json` which excludes autogenerated `.next` files
- This prevents false errors from stale Next.js type definitions
- Your IDE will still use `tsconfig.json` for full type checking

**Output example:**

```bash
🔍 Running TypeScript type checks...
Using tsconfig.precommit.json (excludes autogenerated .next files)
